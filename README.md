...
斗鱼/虎牙直播 DVR 回放工具。基于 PyQt6 + mpv + FFmpeg，支持直播预览、弹幕、时移回看、片段导出。

## 功能

- **多平台支持** — 斗鱼 + 虎牙，UI 顶部切换
- **直播预览** — mpv 实时播放直播流，支持全屏模式（鼠标不动自动隐藏 UI）
- **弹幕采集** — Node.js 连接斗鱼/虎牙弹幕 WebSocket，实时采集展示，录制为 NDJSON
...
- **设置面板** — 可调缓存时长/弹幕字号/透明度等，写入 config.json
- **暗色主题** — 中文 UI，斗鱼风格配色
...
│   ├── ffmpeg.exe                 # FFmpeg（手动放置）
│   └── node.exe                   # Node.js（手动放置）
├── docs/                          # 开发文档
